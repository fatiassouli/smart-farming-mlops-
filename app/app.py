...
from utils.predictor import (
    ...
    CROP_CLASSES, COUNTRIES, REG_CROPS, CROP_ICONS, get_model_status
)
...
DATASET_CROP_DISTRIBUTION = {
    'rice': 100, 'maize': 100, 'chickpea': 100, 'kidneybeans': 100,
    'pigeonpeas': 100, 'mothbeans': 100, 'mungbean': 100, 'blackgram': 100,
    'lentil': 100, 'pomegranate': 100, 'banana': 100, 'mango': 100,
    'grapes': 100, 'watermelon': 100, 'muskmelon': 100, 'apple': 100,
    'orange': 100, 'papaya': 100, 'coconut': 100, 'cotton': 100,
    'jute': 100, 'coffee': 100
}

DATASET_YIELD_BY_CROP = {
    'Cassava': 9.28, 'Maize': 2.94, 'Plantains and others': 8.63,
    'Potatoes': 15.01, 'Rice, paddy': 3.02, 'Sorghum': 1.71,
    'Soybeans': 1.42, 'Sweet potatoes': 8.99, 'Wheat': 2.46, 'Yams': 8.09
}
...
def rate_yield(yield_tha):
    if yield_tha < 1:    return 1, 'Faible'
    if yield_tha < 2.5:  return 2, 'Moyen'
    if yield_tha < 4:    return 3, 'Bon'
    if yield_tha < 6:    return 4, 'Très bon'
    return 5, 'Excellent'
...
def home():
    ...
    CROPS_DATA = {
        'rice':        {'description': 'Variété à haut rendement pour conditions humides', 'avg_yield': 4.5},
        'wheat':       {'description': 'Blé résistant à la sécheresse, climat tempéré',   'avg_yield': 3.2},
        'maize':       {'description': 'Maïs à croissance rapide, rendement élevé',        'avg_yield': 5.8},
        'cotton':      {'description': 'Coton de qualité supérieure pour le textile',       'avg_yield': 2.1},
        'coffee':      {'description': 'Café Arabica pour le marché d\'exportation',        'avg_yield': 1.5},
        'banana':      {'description': 'Banane tropicale sucrée pour le marché local',      'avg_yield': 35.0},
        'mango':       {'description': 'Mangue Alphonso à haute valeur marchande',          'avg_yield': 12.0},
        'coconut':     {'description': 'Noix de coco multi-usage pour divers produits',     'avg_yield': 80.0},
        'chickpea':    {'description': 'Pois chiche riche en protéines, sol aride',         'avg_yield': 1.2},
        'kidneybeans': {'description': 'Haricots rouges riches en nutriments',              'avg_yield': 1.5},
        'lentil':      {'description': 'Lentilles légumineuses à haute valeur nutritive',   'avg_yield': 1.0},
        'pomegranate': {'description': 'Grenade antioxydante à haute valeur',               'avg_yield': 8.0},
        'grapes':      {'description': 'Raisins pour vins fins et raisins secs',            'avg_yield': 15.0},
        'watermelon':  {'description': 'Pastèque rafraîchissante, climat chaud',            'avg_yield': 20.0},
        'muskmelon':   {'description': 'Melon savoureux pour marchés locaux',               'avg_yield': 14.0},
        'apple':       {'description': 'Pomme croquante pour exportation',                  'avg_yield': 25.0},
        'orange':      {'description': 'Orange riche en vitamine C, demande élevée',        'avg_yield': 20.0},
        'papaya':      {'description': 'Papaye tropicale riche en enzymes digestives',      'avg_yield': 35.0},
        'pigeonpeas':  {'description': 'Pois d\'Angole, légumineuse résistante',            'avg_yield': 1.3},
        'mothbeans':   {'description': 'Haricot mite, excellente résistance sécheresse',    'avg_yield': 0.9},
        'mungbean':    {'description': 'Haricot mungo, germination rapide',                  'avg_yield': 1.5},
        'blackgram':   {'description': 'Gramme noir riche en protéines végétales',          'avg_yield': 1.0},
        'jute':        {'description': 'Jute fibre naturelle pour textiles et emballages',  'avg_yield': 2.5},
    }
...
def api_predict():
    ...
    crop_name, confidence, icon = predict_crop(N, P, K, temperature, humidity, ph, rainfall)
...
def dashboard_stats():
    ...
    if preds:
        ...
        top_crops_dataset = [
            ('Rice', 100), ('Maize', 100), ('Cotton', 100),
            ('Coffee', 100), ('Banana', 100), ('Wheat', 100),
            ('Coconut', 100), ('Mango', 100)
        ]
    ...
    trend_labels = ['1961-1970', '1971-1980', '1981-1990', '1991-2000', '2001-2010', '2011-2016']
    ...
    _data_candidates = [
        ...
        _os.path.join(_os.path.dirname(BASE_DIR), 'projet_ML_akhirversion', 'data', 'Crop_recommendation.csv'),
        _os.path.join(_os.path.dirname(BASE_DIR), 'data', 'Crop_recommendation.csv'),
    ]
    crop_csv = next((p for p in _data_candidates if _os.path.exists(p)), _data_candidates[0])
    try:
        ...
        climate_means = {
            'Température (°C)': 25.6, 'Humidité (%)': 71.5,
            'Pluie/10 (mm)': 10.3, 'pH×10': 62.0,
            'Azote (N)': 50.6, 'Phosphore (P)': 53.4, 'Potassium (K)': 48.1
        }
    ...
    avg_yield = round(sum(p.get('yield_tha', 0) for p in preds) / max(len(preds), 1), 2) if preds else round(sum(DATASET_YIELD_BY_CROP.values()) / len(DATASET_YIELD_BY_CROP), 2)
    avg_conf  = round(sum(p.get('confidence', 0) for p in preds) / max(len(preds), 1), 1) if preds else 0
