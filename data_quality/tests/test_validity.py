...
# yield.csv
# ---------------------------------------------------------------------------
@pytest.mark.parametrize("col_name", ["Area Code", "Element Code", "Item Code", "Year Code", "Year", "Value"])
def test_yield_numeric_dtype(yield_df, col_name):
    assert pd.api.types.is_integer_dtype(yield_df[col_name]), (
...
def test_yield_value_range(yield_df, yield_schema):
    ...
    _assert_numeric_range(yield_df, "Value", spec.min_value, spec.max_value, "yield.csv")
...
def test_yield_no_negative_value(yield_df):
    assert (yield_df["Value"] >= 0).all(), "yield.csv : des rendements négatifs ont été détectés"
...
def test_crop_integer_dtype(crop_df, col_name):
    assert pd.api.types.is_integer_dtype(crop_df[col_name]), (
