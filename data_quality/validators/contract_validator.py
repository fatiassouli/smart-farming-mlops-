...
def _validate_column(df, col_spec, dataset_name, violations):
    ...
    if name not in df.columns:
        violations.append(
            Violation(
                dataset_name, name, "missing_column", "Colonne absente du DataFrame"
            )
        )
        return
...
def validate_dataset(dataset_name: str, contracts: dict = None) -> ValidationReport:
    ...
    if not os.path.exists(csv_path):
        raise FileNotFoundError(
            f"Fichier non trouvé: {csv_path} (vérifier data_contracts.yml)"
        )
...
def main():
    ...
    parser.add_argument(
        "--dataset", help="Nom du dataset à valider (yield | crop_recommendation)"
    )
    parser.add_argument(
        "--all", action="store_true", help="Valider tous les datasets du contrat"
    )
    ...
    for name in targets:
        try:
            ...
            print(
                f"[{status}] {name} - {report.n_rows} lignes - {len(report.violations)} violation(s)"
            )
