...
def unit_tests(context: AssetExecutionContext):
    ...
    if result.returncode != 0:
        raise Failure(
            description="Tests unitaires échoués",
            metadata={"stderr": result.stderr[:500]},
        )
...
def code_quality(context: AssetExecutionContext):
    ...
    ruff = subprocess.run(
        ["ruff", "check", "."], capture_output=True, text=True, cwd=str(ROOT_DIR)
    )
    black = subprocess.run(
        ["black", "--check", "."], capture_output=True, text=True, cwd=str(ROOT_DIR)
    )
...
def security_scan(context: AssetExecutionContext):
    ...
    return Output(
        value={"secure": True},
        metadata={"issues": len(issues) if "issues" in locals() else 0},
    )
...
def raw_data(context: AssetExecutionContext):
    ...
    if result.returncode != 0:
        raise Failure(
            description="Ingestion échouée", metadata={"stderr": result.stderr[:500]}
        )
...
def transformed_tables(context: AssetExecutionContext):
    ...
    if result.returncode != 0:
        raise Failure(
            description="dbt échoué", metadata={"stderr": result.stderr[:500]}
        )
...
def data_drift_check(context: AssetExecutionContext):
    ...
    if baseline_path.exists():
        ...
        for dataset, stats in current_stats.items():
            ...
            if base:
                row_diff = abs(stats["n_rows"] - base.get("n_rows", 0)) / max(
                    base.get("n_rows", 1), 1
                )
                if row_diff > 0.1:
